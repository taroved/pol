...
from settings import DATABASES, DOWNLOADER_USER_AGENT, DEBUG
...
def element_to_string(element, html=False):
    s = [element.text] if element.text else []
    for sub_element in element:
        if html:
            s.append(etree.tostring(sub_element))
        else:
            s.append(unicode(etree.tostring(sub_element, encoding='utf-8', method='text'), 'utf-8'))
    if element.tail:
...
def _buildFeed(response, feed_config):
    ...
    for node in tree.xpath(feed_config['xpath']):
        ...
        for field_name in feed_config['fields']:
            field = feed_config['fields'][field_name]
            element = node.xpath(field['xpath'])
            content = None
            if element: # element found
                if field['content_type'] == 'text':
                    content = element_to_string(element[0])
                elif field['content_type'] == 'html':
                    content = element_to_string(element[0], true)
                elif field['content_type'] == 'link':
                    anchor = element[0].xpath('ancestor-or-self::node()[name()="a"]')
                    if anchor and anchor[0].get('href'):
                        content = _build_link(response.body_as_unicode(), feed_config['uri'], anchor[0].get('href'))
                elif field['content_type'] == 'image':
                    if 'src' in element[0].attrib:
                        content = _build_link(response.body_as_unicode(), feed_config['uri'], element[0].get('src'))
    ...
    for item in items:
        feed.add_item(
            ...
            link = item['link'] if 'link' in item else feed_config['uri'],
            description=item['description'] if 'description' in item else '', 
            enclosure=Enclosure(item['image'], "32000", "image/jpeg") if 'image' in item else None, #"Image"
...
def startRssRequest(request, feed_id):
    # get url, xpathes
    creds = DATABASES['default']
    db = MySQLdb.connect(host=creds['HOST'], port=int(creds['PORT']), user=creds['USER'], passwd=creds['PASSWORD'],
                         db=creds['NAME'], cursorclass=MySQLdb.cursors.DictCursor)
    feed = {}
    with db:
        cur = db.cursor()
        cur.execute("""select f.uri, f.xpath, fi.name, ff.xpath as fxpath, ff.content_type, ff.required from frontend_feed f
        ...
        for row in rows:
            if not feed:
                feed['uri'] = row['uri']
                feed['xpath'] = row['xpath']
                feed['fields'] = {}
            feed['fields'][row['name']] = {'xpath': row['fxpath'], 'content_type': row['content_type'], 'required': row['required']}
